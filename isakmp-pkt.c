	...
	size_t data_len, int * reject, enum isakmp_ipsec_proto_enum decode_proto)
{
	...
	if (type & 0x8000) {
		...
		r->af = isakmp_attr_16;
		r->u.attr_16 = length;
		if ((((ISAKMP_XAUTH_ATTRIB_RFC_TYPE <= r->type)
			&& (r->type <= ISAKMP_XAUTH_ATTRIB_RFC_ANSWER)
			&& (r->type != ISAKMP_XAUTH_ATTRIB_RFC_STATUS))
			|| ((ISAKMP_XAUTH_ATTRIB_NORTEL_TYPE <= r->type)
			&& (r->type <= ISAKMP_XAUTH_ATTRIB_NORTEL_ANSWER)
			&& (r->type != ISAKMP_XAUTH_ATTRIB_NORTEL_STATUS)))
			&& (length > 0)
			&& (opt_debug < 99))
		...
		if ((((ISAKMP_XAUTH_ATTRIB_RFC_TYPE <= type)
			&& (type <= ISAKMP_XAUTH_ATTRIB_RFC_ANSWER)
			&& (type != ISAKMP_XAUTH_ATTRIB_RFC_STATUS))
			|| ((ISAKMP_XAUTH_ATTRIB_NORTEL_TYPE <= type)
			&& (type <= ISAKMP_XAUTH_ATTRIB_NORTEL_ANSWER)
			&& (type != ISAKMP_XAUTH_ATTRIB_NORTEL_STATUS)))
			&& (length > 0)
			&& (opt_debug < 99))
